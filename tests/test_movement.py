...
def test_king_legal_moves(tr, tc):
    ...
    assert king_can_move('K', 'w', 0, 0, tr, tc, board) is True
...
def test_king_illegal_moves(tr, tc):
    board = _board([['wK', '.', '.'], ['.', '.', '.'], ['.', '.', '.']])
    assert king_can_move('K', 'w', 0, 0, tr, tc, board) is False
...
def test_queen_legal_moves(tr, tc):
    board = _board([['.'] * 4, ['.'] * 4, ['.'] * 4, ['.'] * 4])
    assert queen_can_move('Q', 'w', 0, 0, tr, tc, board) is True


@pytest.mark.parametrize("tr,tc", [(1, 2), (2, 1)])
def test_queen_illegal_moves(tr, tc):
    board = _board([['.'] * 4, ['.'] * 4, ['.'] * 4, ['.'] * 4])
    assert queen_can_move('Q', 'w', 0, 0, tr, tc, board) is False
...
def test_pawn_legal_moves(color, fr, fc, tr, tc, token_rows):
    ...
    assert pawn_can_move('P', color, fr, fc, tr, tc, board) is True
...
def test_pawn_illegal_moves(color, fr, fc, tr, tc, token_rows):
    board = _board(token_rows)
    assert pawn_can_move('P', color, fr, fc, tr, tc, board) is False
...
def test_validator_returns_false_for_unknown_piece():
    mv = MoveValidator()
    board = _board([['.'] * 3])
    assert mv.is_legal('X', 'w', 0, 0, 0, 1, board) is False
